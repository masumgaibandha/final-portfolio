...
import { getSecurityEnv, isRegistrationOperationallyReady } from "@/lib/env";
...
/* Same rationale as the registrations route: the `mongodb` driver needs Node, and this must never be statically cached. */
 ...
 * `REVIEW` — it can never set `PAID` itself. Guarded by the same
 * registration-open/origin/IP-rate-limit checks as the registration route;
 * the order's own unguessable `publicOrderRef` (from the registration
 * response) acts as the request's bearer capability, so no second Turnstile
 * challenge is required for this step.
 */
export async function POST(
  ...
  if (!senderNumberE164) {
    /* Unreachable in practice — the schema already checked this. */
    return noStoreJson(
