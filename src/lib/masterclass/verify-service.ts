import { getMetaCapiEnv } from "@/lib/env";
 ...
 * admin approve Server Action. The DB transition itself
 * (`verifyPayment()`) is a single atomic `findOneAndUpdate` guarded on
 * `status: "REVIEW"` — a retried/duplicate approval click matches nothing
 * the second time and this function reports `already_processed` rather
 * than re-running any side effect. Email and Meta CAPI are attempted
 * *after* the transition is durable, never inside it, and a failure in
 * either one is recorded but never rolls back `PAID`.
 */
