 ...
 * serializes competing callers through MongoDB itself, the same pattern
 * `rate-limit.ts` already uses for its atomic counters.
 */
...
/**
 * Human-friendly, unique, sequential registration reference — what a student
 * and the admin verification page both refer to a registration by. One
 * sequence per calendar year, so `MC-2026-000001` doesn't collide with a
