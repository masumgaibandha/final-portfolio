...
/** Native <details>/<summary> — no Client Component needed for an accordion. */
export function Faq() {
  return (
      ...
      <div className="border-hairline divide-hairline mt-8 divide-y border-t border-b">
        {faqItems.map((item) => (
          <details key={item.id} className="group py-5">
            <summary
              className="font-bengali text-ink focus-visible:outline-action flex cursor-pointer list-none items-center justify-between gap-4 text-base font-medium marker:content-none focus-visible:outline-2 focus-visible:outline-offset-4 md:text-lg"
            >
              {item.question}
              <LuPlus
