...
import { LuCheck, LuCircleAlert, LuCircleCheck, LuCopy } from "react-icons/lu";
...
/* `Window.fbq` is declared once, in MetaPixel.tsx — this form only ever calls it, never (re)declares its type. */
...
function captureAttribution() {
  ...
  /* Set by the Meta Pixel base script (see MetaPixel.tsx) once it's live — read here rather than duplicating Meta's own cookie logic. */
...
interface MasterclassRegistrationFormProps {
  siteKey: string;
  priceBDT: number;
  paymentMethods: ManualPaymentEnvProp;
}
  ...
  async function onSubmitRegistration(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.status === 201 && body && "publicRegistrationRef" in body) {
        ...
        window.fbq?.("track", "InitiateCheckout", { currency: "BDT", value: priceBDT });
        setStep("payment");
        return;
      }
            ...
            {method && selectedNumber ? (
              <div className="border-hairline bg-canvas mt-5 space-y-4 border p-4">
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  >
                    {copied ? <LuCheck className="size-4" aria-hidden="true" /> : <LuCopy className="size-4" aria-hidden="true" />}
