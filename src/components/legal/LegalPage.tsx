...
/**
 * Shared chrome for all three legal pages — header, hero, body sections,
 * cross-links to the other two policies, contact link, footer. Every
 * page.tsx in the `(legal)` route group only supplies its own `metadata`
 * and this component; none of them duplicate the markup.
 */
              ...
              <section key={section.id} id={section.id} aria-labelledby={`${section.id}-heading`}>
                ...
                <div className="mt-3 space-y-3">
                  {section.paragraphs.map((paragraph) => (
                    <p
                      key={paragraph.slice(0, 24)}
                      className="text-ink-muted font-bengali leading-relaxed"
                    >
                      {paragraph}
                    </p>
                  ))}
                  ...
                  <ul className="mt-3 space-y-2">
                    {section.items.map((item) => (
                      <li key={item} className="text-ink-muted font-bengali flex items-start gap-2.5 leading-relaxed">
                        <span
                          aria-hidden="true"
                          className="bg-action mt-2.5 size-1 shrink-0 rounded-full"
                        />
                        {item}
                      </li>
                    ))}
